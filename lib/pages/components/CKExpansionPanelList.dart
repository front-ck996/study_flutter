...
class CKExpansionPanel {
  ...
  final Function(BuildContext context, bool isExpanded) headerBuilder;
...
/// expansions.
///
/// Note that [expansionCallback] behaves differently for [CKExpansionPanelList]
/// and [CKExpansionPanelList.radio].
///
/// {@tool dartpad}
/// Here is a simple example of how to implement CKExpansionPanelList.
///
...
///
///  * [ExpansionPanel]
///  * [CKExpansionPanelList.radio]
///  * <https://material.io/design/components/lists.html#types>


class CKExpansionPanelList extends StatefulWidget {
  /// Creates an expansion panel list widget. The [expansionCallback] is
  /// triggered when an expansion panel expand/collapse button is pushed.
  ///
  /// The [children] and [animationDuration] arguments must not be null.
  const CKExpansionPanelList({
    super.key,
    this.children = const <CKExpansionPanel>[],
  ...
  ///
  /// {@tool dartpad}
  /// Here is a simple example of how to implement CKExpansionPanelList.radio.
  ///
  /// ** See code in examples/api/lib/material/expansion_panel/expansion_panel_list.expansion_panel_list_radio.0.dart **
  /// {@end-tool}
  const CKExpansionPanelList.radio({
    super.key,
    this.children = const <CKExpansionPanelRadio>[],
    this.expansionCallback,
    this.animationDuration = kThemeAnimationDuration,
  ...
  /// fashion to [ListBody].
  final List<CKExpansionPanel> children;

  /// The callback that gets called whenever one of the expand/collapse buttons
  /// is pressed. The arguments passed to the callback are the index of the
  /// pressed panel and whether the panel is currently expanded or not.
  ///
  /// If CKExpansionPanelList.radio is used, the callback may be called a
  /// second time if a different panel was previously open. The arguments
  /// passed to the second callback are the index of the panel that will close
  /// and false, marking that it will be closed.
  ///
  /// For CKExpansionPanelList, the callback needs to setState when it's notified
  /// about the closing/opening panel. On the other hand, the callback for
  /// CKExpansionPanelList.radio is simply meant to inform the parent widget of
  ...
  /// only used when initializing with the [CKExpansionPanelList.radio]
  /// constructor.)
  ...
  @override
  State<StatefulWidget> createState() => _CKExpansionPanelListState();
}

class _CKExpansionPanelListState extends State<CKExpansionPanelList> {
  ...
  @override
  void didUpdateWidget(CKExpansionPanelList oldWidget) {
    super.didUpdateWidget(oldWidget);

    if (widget._allowOnlyOnePanelOpen) {
      assert(_allIdentifiersUnique(), 'All ExpansionPanelRadio identifier values must be unique.');
      // If the previous widget was non-radio CKExpansionPanelList, initialize the
  ...
  Widget build(BuildContext context) {
    ...
    for (int index = 0; index < widget.children.length; index += 1) {
      ...
      final CKExpansionPanel child = widget.children[index];
